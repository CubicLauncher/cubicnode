...
class MinecraftInstance {
  ...
  getPid(): number {
    return this.process.pid ?? -1; // Devuelve -1 si no hay pid
  }
  ...
  write(input: string): void {
    const stdin: Writable | null = this.process.stdin;
...
class Launcher {
  constructor(MinecraftDir: string) {
    ...
    this.downloader = new Downloader(MinecraftDir);
    this.gameDir = path.resolve(MinecraftDir);
  }

  private createProfile() {
    if (!fs.existsSync(path.resolve(this.gameDir, "launcher_profiles.json"))) {
      fs.writeFileSync(
        path.resolve(this.gameDir, "launcher_profiles.json"),
        JSON.stringify({ profiles: {} }),
      );
    }
  }

  private getJarFiles(directorio, files, ver) {
  ...
  private auth(username: string): string {
  ...
  async launch(options: launchOptions) {
    ...
    const rootPath = this.gameDir;
    const version = options.version;
    const custom = options.version !== version ? options.version : null;
    const username = options.username;
    let java = options.java.Java17;
    let java8 = options.java.Java8;

    const file = JSON.parse(
      fs.readFileSync(
        path.resolve(this.downloader.versions, version, `${version}.json`),
        { encoding: "utf-8" },
      ),
    );

    const uuid = this.auth(username);
    ...
    let gameArgs = [
      ...
      rootPath,
      "--assetsDir",
      path.resolve(this.downloader.assets),
      "--assetIndex",
      version,
      "--uuid",
      uuid,
    ...
    if (custom) {
      ...
      if (!customFile.arguments) {
        ...
        gameArgs.push(
          ...customFile.arguments.game.flatMap((arg) =>
    ...
    jvm = jvm.map((str) =>
      str.replace(/\$\{(\w+)\}/g, (_, p1) => fields[p1] || str),
    );
    ...
    if (
      custom &&
      custom.includes("forge") &&
      parseInt(version.split(".")[1]) < 16 &&
      !java8
    ) {
    ...
    this.emisor.emit(
      "debug",
      `INICIANDO MINECRAFT VERSION: ${custom || version}`,
    );
    ...
    minecraft.on("close", (code) => this.emisor.emit("close", code));
